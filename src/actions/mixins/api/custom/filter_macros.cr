...
module Api::Custom::FilterMacros
  ...
  macro number_chain_filters_for_relation(query, relation_name, *columns)
    {% for column in columns %}
      %eq_sym = :{{relation_name}}__{{column}}
      %lte_sym = :{{relation_name}}__{{column}}__lte
      %gte_sym = :{{relation_name}}__{{column}}__gte
      if params.get?(%eq_sym)
        if params.get(%eq_sym).to_i?
          {{query}} = {{query}}.{{column}}.eq(params.get(%eq_sym))
        end
  ...
  macro string_chain_filters_for_relation(query, relation_name, *columns)
    {% for column in columns %}
      %eq_sym = :{{relation_name}}__{{column}}
      %icontains_sym = :{{relation_name}}__{{column}}__icontains
      if params.get?(%eq_sym)
        {{query}} = {{query}}.{{column}}.eq(params.get(%eq_sym))
      end
      if params.get?(%icontains_sym)
  ...
  macro datetime_chain_filters_for_relation(query, relation_name, *columns)
    {% for column in columns %}
      %eq_sym = :{{relation_name}}__{{column}}
      if params.get?(%eq_sym)
        {{query}} = {{query}}.{{column}}.as_date.eq(params.get(%eq_sym))
  ...
  macro bool_chain_filters_for_relation(query, relation_name, *columns)
    {% for column in columns %}
      %eq_sym = :{{relation_name}}__{{column}}
      if params.get?(%eq_sym)
        {{query}} = {{query}}.{{column}}.eq(params.get(%eq_sym))
  ...
  macro geo_point_chain_filters_for_relation(query, relation_name, *columns)
    {% for column in columns %}
      %longitude_eq_sym = :{{relation_name}}__{{column}}__longitude
      %latitude_eq_sym = :{{relation_name}}__{{column}}__latitude
      %longitude_lte_sym = :{{relation_name}}__{{column}}__longitude_lte
      %longitude_gte_sym = :{{relation_name}}__{{column}}__longitude_gte
      %latitude_lte_sym = :{{relation_name}}__{{column}}__latitude_lte
      %latitude_gte_sym = :{{relation_name}}__{{column}}__latitude_gte
      if params.get?(%longitude_eq_sym)
        if params.get(%longitude_eq_sym).to_f?
          {{query}} = {{query}}.{{column}}.x.eq(params.get(%longitude_eq_sym))
        end
  ...
  macro geo_point_distance_filters_for_relation(query, relation_name, *columns)
    %distance_longitude_sym = :{{relation_name}}__distance_longitude
    %distance_latitude_sym = :{{relation_name}}__distance_latitude
    %distance_radius_gte_sym = :{{relation_name}}__distance_radius_gte
    %distance_radius_lte_sym = :{{relation_name}}__distance_radius_lte
    {% for column in columns %}
      if params.get?(%distance_longitude_sym) && params.get?(%distance_latitude_sym) && params.get?(%distance_radius_lte_sym)
        {{query}} = {{query}}.{{column}}.distance(params.get(%distance_latitude_sym), params.get(%distance_latitude_sym)).lte(params.get(%distance_radius_lte_sym))
      end
  ...
  macro ordering_single_column_query_for_relation(query, relation_name, ordering_sym, default_column, *allowed_columns)
    if params.get?({{ordering_sym}})
      %ordering = params.get({{ordering_sym}})
      %desc = %ordering[0] == '-'
      %order_column = %ordering[0] == '-' ? %ordering.lchop : %ordering
      %order_column = %order_column.split("__")[-1]
      {% for column in allowed_columns %}
        if %order_column == "{{column}}"
          if %desc
            {{query}} = {{query}}.{{column}}.desc_order
